...
export class KeyValueList {
	constructor(
		private plugin: KeyValueListPlugin,
	...
	async load() {
		...
		// const parser = new ListParser(this.plugin);
			...
			ViewPlugin.fromClass(
				class KVLPlugin implements PluginValue {
					...
					constructor(view: EditorView) {
						// this.parser = new ListParser(plugin);
						this.view = view;
						this.listWidths = [];
					...
					private setupEditor = () => {
						const editor =
							this.view.state.field(editorInfoField)?.editor;
						if (!editor) {
					...
					update(update: ViewUpdate) {
						...
						if (
							...
							this.listWidths[this.lastTouchedListIndex] = {
								key: 0,
								row: 0,
							};
							this.needsUpdate = true;
						...
						lists.forEach((list: List, index: number) => {
							...
							syntaxTree(view.state).iterate({
								...
								enter(node: SyntaxNodeRef) {
									listIndex++;
									const nodeLine = view.state.doc.lineAt(
										node.from
									);
									if (
